  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/generate-premium", {
        ...
        body: JSON.stringify({ reportType: "karma", locale }),
      });
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 pb-12">
        ...
        {aiReport && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            className="bg-gradient-to-br from-gold/5 via-amber-900/10 to-purple-900/10 backdrop-blur-xl border border-gold/20 rounded-3xl p-6 sm:p-8 shadow-[0_0_40px_rgba(212,175,55,0.1)]">
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-gold/10 rounded-full"><Sparkles className="w-5 h-5 text-gold" /></div>
              <h3 className="font-serif text-xl font-bold bg-gradient-to-r from-gold via-amber-300 to-gold bg-clip-text text-transparent">AI 카르마 리포트</h3>
            </div>
