...
// SAMPLES_ONLY=premium_naming 처럼 지정하면 해당 샘플만 다시 만든다(쉼표로 여러 개).
import fs from "fs";
...
const only = (process.env.SAMPLES_ONLY ?? "").split(",").map((s) => s.trim()).filter(Boolean);
...
if (want("premium_date_selection")) {
